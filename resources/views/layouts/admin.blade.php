...
<head>
    ...
    <title>{{ config('app.name', 'CSRTS') }} - Admin</title>
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="flex items-center justify-between h-20 px-6 border-b border-slate-800/50 bg-slate-900/50">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 group">
                    ...
                    <span class="text-xl font-bold text-white tracking-tight">CSRTS <span class="text-teal-500">Admin</span></span>
                </a>
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
                ...
                <a href="{{ route('admin.dashboard') }}" 
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-gradient-to-r from-teal-500/20 to-emerald-500/10 text-teal-400 border border-teal-500/20 shadow-inner' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200' }}">
                ...
                <a href="{{ route('admin.complaints.index') }}" 
                   class="flex items-center justify-between px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('admin.complaints.*') ? 'bg-gradient-to-r from-teal-500/20 to-emerald-500/10 text-teal-400 border border-teal-500/20 shadow-inner' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200' }}">
                ...
                <a href="{{ route('admin.users.index') }}" 
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('admin.users.*') ? 'bg-gradient-to-r from-teal-500/20 to-emerald-500/10 text-teal-400 border border-teal-500/20 shadow-inner' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200' }}">
                    <i class="fas fa-users w-5 text-center"></i>
                    <span class="font-medium">Users</span>
                </a>

                <a href="{{ route('admin.categories.index') }}" 
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('admin.categories.*') ? 'bg-gradient-to-r from-teal-500/20 to-emerald-500/10 text-teal-400 border border-teal-500/20 shadow-inner' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200' }}">
                    ...
                    <div class="relative" x-data="{ profileOpen: false }">
                        <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false" class="flex items-center gap-3 focus:outline-none bg-white hover:bg-slate-50 border border-slate-200 rounded-xl p-1.5 pl-2 transition-colors shadow-sm">
                            <div class="hidden sm:block text-right">
                                ...
                                <p class="text-[10px] text-slate-500 font-medium uppercase tracking-wider">Administrator</p>
                            </div>
