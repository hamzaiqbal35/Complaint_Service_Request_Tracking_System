    ...
    <div class="p-8">
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
            ...
            <div>
                ...
                @error('email')
                    <div class="mt-2 text-rose-400 text-sm">{{ $message }}</div>
                @enderror
            </div>
            ...
            <!-- Confirm Password -->
            <div>
                <label for="password-confirm" class="block text-sm font-medium text-slate-300 mb-2">Confirm Password</label>
                <div class="relative">
                    <input id="password-confirm" type="password" name="password_confirmation" required autocomplete="new-password"
                        class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition-colors pr-12">
                    <button type="button" class="absolute inset-y-0 right-0 px-4 flex items-center text-slate-400 hover:text-white transition-colors" onclick="togglePasswordVisibility('password-confirm', this)">
                        <i class="far fa-eye" id="eye-icon-password_confirm"></i>
                    </button>
                </div>
            </div>
